<template>
  <div class="card-detail min-h-screen bg-gray-50 p-4 dark:bg-gray-900">
    <button class="back-button mb-4 text-blue-600 hover:underline dark:text-blue-400" @click="goBack" aria-label="Go back to News & Stories page">
    ...
    <div v-if="item" class="bg-white rounded-xl shadow-md p-6 dark:bg-gray-800">
      ...
      <div v-if="item.type === 'story'" class="person-layout">
        <div class="relative mb-8 md:mb-12 h-72 md:h-[450px] rounded-lg overflow-hidden shadow-lg">
          <img :src="item.avatar" :alt="`Portrait of ${item.name} graduating`" class="w-full h-full object-cover object-center" loading="lazy" />
        ...
        <div class="mb-8 md:mb-12">
          ...
          <div class="space-y-8">
            <div v-for="(story, index) in item.fullStory" :key="index" class="bg-white p-6 rounded-lg shadow-sm dark:bg-gray-700">
        ...
        <div class="mb-8 md:mb-12">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="(image, index) in item.images" :key="index" class="rounded-lg overflow-hidden shadow-md">
              <img :src="image" :alt="`Image of ${item.name} ${index + 1}`" class="w-full h-64 object-cover object-center transform hover:scale-105 transition-transform duration-300" loading="lazy" />
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="container mx-auto px-4 max-w-7xl relative z-10">
            <div class="flex items-center">
              <div class="w-16 h-16 rounded-full bg-blue-200/20 flex items-center justify-center mr-6">
                <i :class="item.icon || 'fas fa-question'" class="text-2xl text-white"></i>
              </div>
              <div>
          ...
          <div class="container mx-auto px-4 max-w-7xl">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div class="lg:col-span-2 space-y-8">
                <section>
                  <h2 class="text-2xl font-poppins font-semibold text-gray-900 mb-4 dark:text-white">About This Program</h2>
              ...
              <div class="space-y-8">
                <section v-if="item.stats" class="bg-gray-50 rounded-lg p-6 dark:bg-gray-700">
                  ...
                  <div class="space-y-4">
                    ...
                    <div
                      v-if="!(item.stats && item.stats.length)"
                      class="text-gray-500 text-sm dark:text-gray-400"
                    >
          ...
          <div class="p-8 lg:p-12">
            <div class="mb-8">
              <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <span class="text-sm font-medium text-gray-600 dark:text-gray-400 text-base sm:text-lg md:text-xl lg:text-base">
            ...
            <div class="prose prose-lg max-w-none text-gray-700 space-y-6 dark:text-gray-300 dark:prose-invert">
              <p v-for="(paragraph, index) in item.content.split('\n\n')" :key="index" class="md:text-justify text-base sm:text-lg md:text-xl lg:text-base">
                {{ paragraph }}
              </p>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div v-for="(img, index) in item.image1" :key="index">
                  <img
                    ...
                    class="w-full md:h-64 object-cover rounded-lg shadow-md"
                    loading="lazy"
                    @error="handleImageError"
                  />
                </div>
...
<style scoped>
.card-detail { max-width: 1200px; margin: 0 auto; }
.font-poppins { font-family: 'Poppins', sans-serif; }
.back-button { background: none; border: none; cursor: pointer; }
.prose :where(h2):not(:where([class~="not-prose"] *)) { font-size: 1.5rem; }
.prose :where(p):not(:where([class~="not-prose"] *)) { line-height: 1.75; }
</style>