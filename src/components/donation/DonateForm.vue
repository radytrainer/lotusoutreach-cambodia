  ...
  <div class="lg:col-span-3">
    <form @submit.prevent="handleSubmit" class="bg-white rounded-xl shadow-2xl border border-pink-100 p-6">
      ...
      <div class="p-6 space-y-6">
        <div>
          <label for="amount" class="block text-gray-700 text-sm font-medium mb-2 items-center">
          ...
          <div class="relative">
            ...
            <input v-model.number="form.amount"
              ...
              :class="{ 'border-red-500': errors.amount }" type="number" id="amount" min="1" required placeholder="0.00"
              aria-describedby="amount-error" />
            <div class="w-full bg-gray-200 rounded-full h-2 mt-2">
              <div class="bg-pink-600 h-2 rounded-full transition-all duration-300" :style="{ width: progress + '%' }">
              </div>
            </div>
            <p v-if="errors.amount" id="amount-error" class="text-red-500 text-xs mt-1">
              {{ errors.amount }}
            </p>
          </div>
        </div>
        <div>
          <label for="anonymous" class="block text-gray-700 text-sm font-medium mb-2 items-center">
            <svg class="h-5 w-5 mr-2 text-pink-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div>
          <label class="block text-gray-700 text-sm font-medium mb-2 items-center">
            <svg class="h-5 w-5 mr-2 text-pink-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input id="full-name" type="text" v-model.trim="form.fullName" required
                ...
                :class="{ 'border-red-500': errors.fullName }" aria-describedby="full-name-error" />
              <p v-if="errors.fullName" id="full-name-error" class="text-red-500 text-xs mt-1">
                {{ errors.fullName }}
              </p>
            </div>
            ...
            <div>
              ...
              <input id="email" type="email" v-model="form.email" required
                class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500"
                :class="{ 'border-red-500': errors.email }" aria-describedby="email-error" />
              <p v-if="errors.email" id="email-error" class="text-red-500 text-xs mt-1">
                {{ errors.email }}
              </p>
            </div>
            ...
            <div class="md:col-span-2">
              ...
              <select id="country" v-model="form.country" required
                class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-pink-500"
                :class="{ 'border-red-500': errors.country }" aria-describedby="country-error">
                <option value="">Select Country</option>
                <option v-for="country in countries" :key="country" :value="country">
                  {{ country }}
                </option>
              </select>
              <p v-if="errors.country" id="country-error" class="text-red-500 text-xs mt-1">
                {{ errors.country }}
              </p>
            </div>
          </div>
        ...
        <div>
          <label for="comment" class="block text-gray-700 text-sm font-medium mb-2 items-center">
            <svg class="h-5 w-5 mr-2 text-pink-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
// Submit form and send via EmailJS
...
const setAmount = (value) => {
  form.amount = value;
  validateForm(); // Re-validate on amount change
};
</script>
