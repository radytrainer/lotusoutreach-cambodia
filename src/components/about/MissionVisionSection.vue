    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-12">
      ...
      <div>
        ...
        <div ref="principlesContainer">
          <div v-for="(item, index) in principles" :key="index" class="mb-6 flex items-start gap-4">
            <div class="w-10 h-10 flex items-center justify-center rounded-full bg-blue-100">
              <i :class="`${item.icon} text-blue-600 text-xl`"></i>
            </div>
            <div>
              <h3 class="text-lg sm:text-xl font-semibold text-blue-800 mb-1">{{ item.title }}</h3>
      ...
      <!-- Right Column: Carousel (smaller image) -->
      <div
        class="relative overflow-hidden rounded-2xl w-full mx-auto"
        :style="{ height: smallImageHeight }"
      >
...
const smallImageHeight = ref('250px') // fixed small height
